...
function initPricingCalculator() {
  const slider = document.getElementById('calc-videos');
  const displayCount = document.getElementById('calc-count');
  ...
  function updateCalculator() {
    const videos = parseInt(slider.value);
    const filming = document.querySelector('input[name="filming"]:checked');
    const includeFilming = filming && filming.value === 'yes';

    if (displayCount) displayCount.textContent = videos;

    // Cost calculation
    const editingCost = videos * 20;
    const filmingCost = includeFilming ? 150 : 0;
    const total = editingCost + filmingCost;
    ...
    // Breakdown
    if (resultBreakdown) {
      let breakdown = `${videos} videos × €20 = €${editingCost}`;
      if (includeFilming) breakdown += ` + Content creation = €150`;
      resultBreakdown.textContent = breakdown;
    }

    // Update slider gradient
    const max = parseInt(slider.max);
    const pct = ((videos - parseInt(slider.min)) / (max - parseInt(slider.min))) * 100;
    slider.style.background = `linear-gradient(90deg, var(--color-purple-light) ${pct}%, var(--border-medium) ${pct}%)`;
  }

  slider.addEventListener('input', updateCalculator);

  document.querySelectorAll('input[name="filming"]').forEach(radio => {
